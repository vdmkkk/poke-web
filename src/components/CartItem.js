      ...
      return (
          ...
          <div className='maininfo'>
            <div className="info-container">
              <p className="cart-name">Название карточки: {eachCard.name}</p>
              <span>Количество:</span>
              <input min='1' type='number' onChange={(event) => {handleQuantity(event.target.value)}} value={eachCard.quantity}></input>
            </div>
  
            <div className='costContainer'>
              <p className="cart-cost">Цена: <span className='cardcost'>{eachCard.itemCost}₽</span></p>
              <button className="ui button"
                onClick={() => handleRemoveFromCart(eachCard.id, token)}>
                  Удалить {eachCard.name} из корзины
              </button>
            </div>
            </div>
